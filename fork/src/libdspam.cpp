  ...
  u_int32_t flags)
{
  ...
  CTX = static_cast<DSPAM_CTX*>(calloc (1, sizeof (DSPAM_CTX)));
  if (CTX == NULL) {
    LOG(LOG_WARNING, "dspam_create: unable to allocate space for classification context");
    return NULL;
  }

  CTX->config = static_cast<_ds_config*>(calloc(1, sizeof(struct _ds_config)));
  if (CTX->config == NULL) {
  ...
  CTX->config->size = 128;
  CTX->config->attributes = static_cast<attribute **> (calloc(1, sizeof(attribute_t)*(CTX->config->size)));
...
int dspam_addattribute (DSPAM_CTX * CTX, const char *key, const char *value) {
  ...
  if (j >= CTX->config->size) {
    config_t ptr;
    CTX->config->size *= 2;
    ptr = static_cast<attribute **>(realloc(CTX->config->attributes,
...
dspam_process (DSPAM_CTX * CTX, const char *message)
{
  ...
  if (CTX->class_name[0] == 0) {
    if (CTX->result == DSR_ISSPAM)
      strcpy(CTX->class_name, LANG_CLASS_SPAM);
    else if (CTX->result == DSR_ISINNOCENT)
      strcpy(CTX->class_name, LANG_CLASS_INNOCENT);
  }
...
_ds_operate (DSPAM_CTX * CTX, char *headers, char *body)
{
    ...
    && CTX->source != DSS_CORPUS)
  {
    ...
    CTX->signature = static_cast <_ds_spam_signature*>(calloc (1, sizeof (struct _ds_spam_signature)));
    if (CTX->signature == NULL)
    {
    ...
    strcpy(static_cast<char*>(CTX->signature->data), headers);
    strcat(static_cast<char *>(CTX->signature->data), "\001");
    strcat(static_cast<char *>(CTX->signature->data), body);
  }
  ...
  if (CTX->flags & DSF_WHITELIST && do_whitelist) {
    ...
    strcpy(CTX->class_name, LANG_CLASS_WHITELISTED);
  }
...
int _ds_factor(struct nt *set, char *token_name, float value) {
  ...
  f = static_cast<dspam_factor *>(calloc(1, sizeof(struct dspam_factor)));
  if (!f)
    return EUNKNOWN;
  ...
  char identifier)
{
  ...
  while(node_nt != NULL) {
    ds_term = static_cast<_ds_term *>(node_nt->ptr);
