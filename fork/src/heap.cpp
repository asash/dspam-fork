...
ds_heap_create(int size, int type)
{
  ...
  h = static_cast<ds_heap_t>(calloc(1, sizeof(struct _ds_heap)));
  h->size = size;
  h->type = type;
  return h;
}
...
ds_heap_element_create (double probability, 
                  ...
                  int complexity)
{
  ds_heap_element_t element = static_cast<ds_heap_element_t>(calloc(1, sizeof(struct _ds_heap_element)));
