...
agent_pref_t _ds_pref_aggregate(agent_pref_t STX, agent_pref_t UTX) {
  agent_pref_t PTX = static_cast<_ds_agent_attribute **>(calloc(1, PREF_MAX*sizeof(agent_attrib_t )));
  int i, size = 0;
...
agent_attrib_t _ds_pref_new(const char *attribute, const char *value) {
  ...
  pref = static_cast<agent_attrib_t>(malloc(sizeof(struct _ds_agent_attribute)));
  ...
  void *ignore)
{
  ...
  agent_pref_t PTX = static_cast<agent_pref_t>(malloc(sizeof(agent_attrib_t )*PREF_MAX));
  char buff[258];
  FILE *file;
  char *p, *q;
  int i = 0;
