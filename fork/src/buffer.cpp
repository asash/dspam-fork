...
buffer_create (const char *s)
{
  ...
  b = static_cast<buffer *>(malloc (sizeof (buffer)));
  if (!b)
    return NULL;
  ...
  if (!s)
  {
    b->size = 1024;
    b->used = 0;
    b->data = static_cast <char *> (malloc(b->size));
    if (!b->data) 
      return NULL;
    b->data[0] = 0;
    return b;
...
buffer_copy (buffer * b, const char *s)
{
  ...
  len = strlen (s);
  new_data = static_cast<char *>(malloc (len + 1));
  if (new_data == NULL)
    return -1;
...
buffer_cat (buffer * b, const char *s)
{
  ...
  if (used >= size)
  {
    size *= 2;
    size += len;
    new_data = static_cast<char *>(realloc (b->data, size));
    if (!new_data)
      return -1;
    b->data = new_data;
    b->size = size;
