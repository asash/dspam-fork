    ...
    if (block_position == BP_HEADER)
    {
      ...
      else if (line[0] == 32 || line[0] == '\t')
      {
        if (current_heading)
        {
          ...
          ptr = static_cast<char *>(realloc (current_heading->data,
    ...
    for(enc_offset = 0; header->concatenated_data[enc_offset]; enc_offset++)
    {
      ...
      if (!strncmp(enc, "=?", 2)) {
        ...
        if (decoded)
        {
          ...
          new_alloc = static_cast<char *>(calloc (1, offset + decoded_len + strlen (rest) + 2));
          if (new_alloc == NULL) {
            LOG (LOG_CRIT, ERR_MEM_ALLOC);
          }
          else
...
_ds_decode_quoted (const char *body)
{
  ...
  n = out = static_cast<char*>(malloc(strlen(body)+1));
  ...
  while (node_nt != NULL && node_nt->ptr != NULL)
  {
    ...
    if (block->headers != NULL && block->headers->items > 0)
    {
      ...
      while (node_header != NULL)
      {
        ...
        heading = static_cast<char *> (malloc(
...
_ds_push_boundary (struct nt *stack, const char *boundary)
{
  ...
  y = static_cast<char*> (malloc (strlen (boundary) + 3));
  if (y == NULL)
    return EUNKNOWN;
...
_ds_pop_boundary (struct nt *stack)
{
  ...
  boundary = strdup (static_cast<char *>(last_node->ptr));
...
_ds_match_boundary (struct nt *stack, const char *buff)
{
  ...
  while (node != NULL)
  {
    if (!strncmp (buff, static_cast<char *>(node->ptr), strlen(static_cast<char *>(node->ptr))))
    {
      return 1;
    }
    node = c_nt_next (stack, &c);
...
_ds_find_header (ds_message_t message, const char *heading) {
  ...
  if (message->components->first) {
    if ((block = static_cast<_ds_message_part*>(message->components->first->ptr))==NULL)
      return NULL;
...
_ds_strip_html (const char *html)
{
  ...
  html2 = static_cast<char *>(malloc(len+1));
