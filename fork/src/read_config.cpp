...
#ifndef SPLIT_CONFIG
  attrib = static_cast<attribute **> (calloc(1, attrib_size*sizeof(attribute_t)));
  if (attrib == NULL) {
    LOG(LOG_CRIT, ERR_MEM_ALLOC);
    return NULL;
  }
  ...
  while(fgets(buffer, sizeof(buffer), file)!=NULL) {
    ...
    while ((v = tokenize(NULL, &bufptr)) != NULL) {
      ...
      if (_ds_find_attribute(attrib, a)!=NULL) {
        ...
        if (num_root >= attrib_size) {
          attrib_size *=2;
          ptr = static_cast<attribute **>(realloc(attrib, attrib_size*sizeof(attribute_t)));
          if (ptr)
            attrib = ptr;
          else {
            LOG(LOG_CRIT, ERR_MEM_ALLOC);
...
config_t read_config(const char *path) {
  ...
  if (fileread(path, &attrib, num_root) == 0)
    return NULL;
#else
  ptr = static_cast<attribute **>(realloc(attrib, ((num_root+1)*sizeof(attribute_t))+1));
  if (ptr)
    return ptr;
  LOG(LOG_CRIT, ERR_MEM_ALLOC);
#endif
...
{
  agent_pref_t PTX = static_cast<_ds_agent_attribute **>(malloc(sizeof(agent_attrib_t)*PREF_MAX));
  agent_pref_t ptr;
  attribute_t attrib;
  char *p, *q;
  char *ptrptr = NULL;
  ...
  ptr = static_cast<_ds_agent_attribute **>(realloc(PTX, sizeof(agent_attrib_t)*(i+1)));
  if (ptr)
    return ptr;
